...
import { GET_TASKS, GET_USERS, SELECT_ASSIGNEE, UPDATE_PRIORITY, UPDATE_STATUS } from "../Queries"

import { closeModal } from "../../redux/Modal"
import { useDispatch } from "react-redux"

const TaskModalRight = ({ task }) => {
  const [openStatuses, setOpenStatuses] = useState(false)
  const [nextStatus, setNextStatus] = useState('')

  const [currentAssignee, setCurrentAssignee] = useState('Unassigned')
  const [openAssignee, setOpenAssignee] = useState(false)
  const [selectAssignee] = useMutation(SELECT_ASSIGNEE)
  const onSelectAssignee = (username: string, userId: string) => {
    setCurrentAssignee(username)
    selectAssignee({variables: { taskId: task.id, userId }})
  }
  ...
  return (
    <div className="task-modal-right">
      <div className="current-status" style={statusStyle} onClick={() => setOpenStatuses(!openStatuses)}>
      ...
      {openStatuses && <ul className="statuses">
      ...
      <div className="assignee" onClick={onChangeAssignee}>{currentAssignee}
        {openAssignee && <ul className="assignees-list">
          {data.users.map(user => (
            <li key={user.id} onClick={() => onSelectAssignee(user.username, user.id)}>{user.username}</li>
          ))}
        </ul>}
      </div>
      <div className="reporter">{task.reporter.username}</div>
      <div className="current-priority" onClick={() => setOpenPriorities(!openPriorities)}>{priorityIcon} {priority ? priority : task.priority}</div>
      {openPriorities && <ul className="priorities">
